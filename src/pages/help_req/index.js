...
export default function Main() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const storedUsername = localStorage.getItem('guestUsername');
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_DJANGO_API_URL}/helprq/api/request-help/`,
        { phone_number: phoneNumber, username: storedUsername },
        { withCredentials: true }
      );
      if (response.data.status === 'success') {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col items-center justify-center w-full h-[calc(100%-48px)] px-[20px] bg-[#232323]">
        <div className="flex flex-col items-center gap-y-[20px] w-full h-auto bg-[#fff] my-[20px] py-[28px] px-[24px] rounded-[4px] shadow-xl">
          ...
          <input type="number" 
            ...
            required className="p-[16px] bg-[#fff] w-full rounded-[8px] border border-[#c8c8c8] !appearance-none !outline-none focus:border-primary hover:!border-primary" placeholder="-를 제외하고 입력해주세요." />
