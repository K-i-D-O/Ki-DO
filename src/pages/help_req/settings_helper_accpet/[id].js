...
export default function Main() {
  ...
  const startChat = async () => {
    try {
      // SendBird Chat SDK 초기화
      const sb = new SendBird({ appId: process.env.NEXT_PUBLIC_SENDBIRD_APP_ID });

      // 헬퍼 사용자의 인증 및 WebSocket 연결
      const helperUserId = localStorage.getItem("helperUserId");
      await sb.connect(helperUserId);

      // 1:1 채팅방 생성 또는 기존 채팅방 가져오기
      const guestUserId = requestDetails.guest_sendbird_user_id;  // 게스트 사용자의 Sendbird User ID
      const params = new sb.GroupChannelParams();
      params.addUserIds([guestUserId]);
      params.isDistinct = true;  // 동일한 유저간 중복 채널 방지

      sb.GroupChannel.createChannel(params, (groupChannel, error) => {
        if (error) {
          console.error('Failed to create or get channel:', error);
          return;
        }

        console.log('Chat channel created or retrieved:', groupChannel.url);
        setChannel(groupChannel);

        // 채팅방으로 리디렉션 (프론트엔드 라우트 변경에 따라 수정 가능)
        router.push(`/chat/${groupChannel.url}`);
      });
    } catch (error) {
        ...
        <div className="flex flex-col items-center gap-y-[18px] w-full h-auto">
          ...
          <div className="w-full h-auto py-[20px] px-[24px] bg-[#fff] rounded-[4px]">
            <form action="/help_req/settings_helper_main" className="flex flex-col items-center gap-y-[19px] w-full h-auto">
              ...
              {/* 채팅 시작 버튼 */}
              <button type="button" onClick={startChat} className="bg-primary flex items-center justify-center py-[13px] px-[15px] rounded-[48px] text-[#fff] w-full text-[18px] font-[700] tracking-[-0.8px] hover:opacity-70">
                채팅 시작하기
              </button>
